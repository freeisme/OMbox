...
def create_device(client: Client, code: str, name: str, category: str) -> str:
    """Create a datacenter device in stock state and return its id."""
    status, payload = client.request(
        "POST",
        "/api/datacenter-devices",
        {"code": code, "name": name, "category": category, "uHeight": 1, "status": "stock"},
        expected=201,
    )
    return payload["id"]
...
def main() -> int:
    ...
    # 机房设备台账：未上架设备列表只来自这里，办公终端与 IT 物资不再出现
    status, payload = admin.request(
        "POST",
        "/api/datacenter-devices",
        {
            "code": f"{PREFIX.upper()}-DE-{suffix}",
            "name": f"{PREFIX} 服务器",
            "brandModel": f"{PREFIX} Rack Server",
            "category": "server",
            "uHeight": 2,
            "serialNumber": f"{PREFIX.upper()}-SN-{suffix}",
            "assetCode": f"{PREFIX.upper()}-FA-{suffix}",
            "status": "stock",
        },
        expected=201,
    )
    datacenter_device_id = payload["id"]

    status, payload = admin.request("GET", "/api/rack-layout/available", expected=200)
    available = payload.get("devices") or []
    our_device = next((item for item in available if str(item["deviceId"]) == str(datacenter_device_id)), None)
    assert our_device, available[:5]
    assert all(item["name"] != f"{PREFIX}-SRV-{suffix}" for item in available), "办公终端不应出现在未上架设备里"
    print("available devices ok:", len(available), "台机房设备（不含办公终端与 IT 物资）")

    place_payload = {
        "sourceKind": "datacenter",
        "datacenterDeviceId": datacenter_device_id,
        "displayName": our_device["name"],
        "brandModel": our_device["brandModel"],
        "category": "server",
        "positionU": 10,
        "uHeight": 2,
        "face": "front",
    ...
    assert placed and placed["status"] == "installed", payload["devices"]
    assert str(placed["rackId"]) == str(rack_id), placed
    print("place datacenter device ok:", computer_placement_id, placed["status"])
    ...
    admin.request(
        ...
        {
            "sourceKind": "datacenter",
            "datacenterDeviceId": create_device(admin, f"{PREFIX.upper()}-FILL-{suffix}", f"{PREFIX} 占位设备", "server"),
            "positionU": 20,
            "uHeight": 1,
            "category": "server",
        },
    ...
    admin.request(
        ...
        {
            "sourceKind": "datacenter",
            "datacenterDeviceId": create_device(admin, f"{PREFIX.upper()}-CLASH-{suffix}", f"{PREFIX} 重叠测试", "network"),
            "positionU": 10,
            "uHeight": 1,
            "category": "network",
            "face": "front",
    ...
    status, rear = admin.request(
        ...
        {
            "sourceKind": "datacenter",
            "datacenterDeviceId": create_device(admin, f"{PREFIX.upper()}-FAN-{suffix}", f"{PREFIX} 后面板风扇", "cooling"),
            "positionU": 10,
            "uHeight": 1,
            "category": "cooling",
            "face": "rear",
    ...
    admin.request(
        ...
        {
            "sourceKind": "datacenter",
            "datacenterDeviceId": create_device(admin, f"{PREFIX.upper()}-OVER-{suffix}", f"{PREFIX} 超界测试", "server"),
            "positionU": 42,
            "uHeight": 2,
            "category": "server",
        },
    ...
    admin.request(
        ...
        {"positionU": 30, "uHeight": 4, "face": "front", "displayName": our_device["name"]},
        expected=200,
    )
    admin.request(
        "PUT",
        f"/api/rack-layout/placements/{computer_placement_id}",
        {"positionU": 10, "uHeight": 1, "face": "both", "displayName": our_device["name"]},
        expected=409,
    )
    admin.request(
        "PUT",
    ...
    status, payload = admin.request("GET", "/api/rack-layout/available", expected=200)
    available_ids = [str(item["deviceId"]) for item in payload.get("devices") or []]
    assert str(datacenter_device_id) in available_ids, available_ids[:5]
    status, payload = admin.request("GET", f"/api/datacenter-devices?keyword={PREFIX.upper()}-DE-{suffix}", expected=200)
    restored = payload["devices"][0] if payload.get("devices") else {}
    assert restored.get("status") == "stock", restored
    ...
    viewer.request(
        "POST",
        f"/api/rack-layout/racks/{rack_id}/placements",
        {"sourceKind": "datacenter", "datacenterDeviceId": "1", "positionU": 1, "uHeight": 1},
        expected=403,
    )
