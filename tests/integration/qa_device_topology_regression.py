...
SAMPLE_DEVICE_TYPE = {
    ...
    "category": "network",
    "ports": [
        {"name": "GE1", "type": "1000base-t", "kind": "network", "face": "front", "rowIndex": 1, "positionIndex": 1},
        {"name": "GE2", "type": "1000base-t", "kind": "network", "face": "front", "rowIndex": 1, "positionIndex": 2},
        {"name": "XGE1", "type": "10gbase-x-sfpp", "kind": "fiber", "face": "front", "rowIndex": 2, "positionIndex": 1},
        {"name": "PSU1", "type": "iec-60320-c14", "kind": "power", "face": "rear", "rowIndex": 3, "positionIndex": 1},
    ],
}
...
def main() -> int:
    ...
    status, payload = admin.request(
        ...
        "/api/device-types",
        {
            key: (value.format(prefix=PREFIX) if isinstance(value, str) else value)
            for key, value in SAMPLE_DEVICE_TYPE.items()
            if key != "ports"
        }
        | {"ports": SAMPLE_DEVICE_TYPE["ports"]},
        expected=201,
    )
    ...
    status, payload = admin.request(
        "POST",
        f"/api/device-types/{catalog_id}/image",
        {"face": "front", "fileName": "panel.png", "contentBase64": one_pixel_png},
        expected=200,
    )
    image_path = payload["path"]
    assert image_path.startswith("/assets/device-images/") and image_path.endswith(".png"), payload
    disk_path = ROOT / "web" / image_path.lstrip("/")
    assert disk_path.is_file(), disk_path
    status, payload = admin.request("GET", f"/api/device-types/{catalog_id}", expected=200)
    assert payload["deviceType"]["imageFrontPath"] == image_path, payload["deviceType"]
    assert int(payload["deviceType"]["frontImage"]) == 1, payload["deviceType"]
    admin.request(
        "POST",
        f"/api/device-types/{catalog_id}/image",
        {"face": "front", "fileName": "bad.gif", "contentBase64": "R0lGODlhAQABAA=="},
        expected=400,
    )
    admin.request(
        "POST",
        f"/api/device-types/{catalog_id}/image/remove",
        {"face": "front"},
        expected=200,
    )
    assert not disk_path.exists(), disk_path
    status, payload = admin.request("GET", f"/api/device-types/{catalog_id}", expected=200)
    assert payload["deviceType"]["imageFrontPath"] == "", payload["deviceType"]
    print("panel image upload/remove ok:", image_path)
    ...
    switch_device_id = created["id"]
    status, created = admin.request(
        "POST",
        "/api/datacenter-devices",
        {
            "code": f"{PREFIX.upper()}-SRV-{suffix}",
            "name": f"{PREFIX} 服务器",
            "brandModel": f"{PREFIX}Vendor Rack Server",
            "category": "server",
            "uHeight": 2,
            "status": "stock",
        },
    ...
    status, switch = admin.request(
        ...
        f"/api/rack-layout/racks/{rack_id}/placements",
        {
            ...
            "datacenterDeviceId": switch_device_id,
            "positionU": 40,
            "uHeight": 1,
            "face": "front",
        },
    ...
    status, server = admin.request(
        ...
        {
            "sourceKind": "datacenter",
            "datacenterDeviceId": server_device_id,
            "positionU": 30,
            "uHeight": 2,
            "face": "front",
        },
