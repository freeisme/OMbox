...
class ThirdPartyLicenseTests(TestCase):
    ...
    def test_notices_file_has_generated_block_and_attribution(self):
        notices = self._notices()

        self.assertIn("<!-- begin: generated by tools/generate_third_party_notices.py -->", notices)
        self.assertIn("<!-- end: generated -->", notices)
        # 借鉴说明必须点名来源，并说明只借鉴思路、未复制代码或素材。
        self.assertIn("## 借鉴与灵感来源", notices)
        self.assertIn("NetBox", notices)
        self.assertIn("Rackula", notices)
        self.assertIn("没有复制其代码、模板或素材", notices)

    def test_every_production_dependency_is_listed(self):
        manifest = json.loads((ROOT / "frontend" / "package.json").read_text(encoding="utf-8"))
        notices = self._notices()
        lock = (ROOT / "frontend" / "pnpm-lock.yaml").read_text(encoding="utf-8")

        # 直接生产依赖必须逐条登记；CI 里没有 node_modules，传递依赖用 lockfile 兜底核对。
        for name in manifest.get("dependencies") or {}:
            self.assertIn(f"`{name}`", notices)
            self.assertIn(f"{name}@", lock)
        self.assertIn("element-plus@", notices)
        self.assertIn("`dayjs@", notices)

    def test_no_copyleft_license_is_shipped(self):
        notices = self._notices()

        # 专有内部许可 + 宽松第三方许可的组合；出现传染性许可必须先确认。
        for marker in ("GPL", "AGPL", "LGPL", "SSPL", "UNKNOWN"):
            self.assertNotIn(f"### {marker}", notices)
        self.assertIn("### MIT", notices)
        self.assertIn("### BSD-3-Clause", notices)

    def test_python_side_still_uses_the_standard_library_only(self):
        import sys as _sys

        stdlib = set(_sys.stdlib_module_names)
        local = {"office_asset", "server", "tools", "tests"}
        sources = [ROOT / "server.py"]
        sources += sorted((ROOT / "office_asset").glob("*.py"))
            ...
            for node in ast.walk(tree):
                ...
                for name in names:
                    if name not in stdlib and name not in local:
                        offenders[name] = path.relative_to(ROOT).as_posix()

        self.assertEqual(offenders, {}, f"服务端出现第三方依赖：{offenders}")
        requirements = (ROOT / "requirements.txt").read_text(encoding="utf-8")
        for line in requirements.splitlines():
            self.assertTrue(line.startswith("#") or not line.strip(), line)

    def test_release_artifacts_carry_the_license_notices(self):
        dockerfile = (ROOT / "Dockerfile").read_text(encoding="utf-8")
        readme = (ROOT / "README.md").read_text(encoding="utf-8")
        license_text = (ROOT / "LICENSE").read_text(encoding="utf-8")
        generator = (ROOT / "tools" / "generate_third_party_notices.py").read_text(
            encoding="utf-8"
        )
        ...
        # 生成脚本只读依赖、只写清单，不引入新的运行时依赖
        self.assertIn('FRONTEND = ROOT / "frontend"', generator)
        self.assertIn('read_json(FRONTEND / "package.json")', generator)
        self.assertIn("LICENSE_FILE_NAMES", generator)

    def test_scanner_only_exempts_upstream_emails_in_the_notices_file(self):
        scanner = (ROOT / "tools" / "scan_release_safety.py").read_text(encoding="utf-8")

        self.assertIn('EMAIL_RULE_EXEMPT_FILES = {"THIRD_PARTY_NOTICES.md"}', scanner)
        self.assertIn('if path in EMAIL_RULE_EXEMPT_FILES:', scanner)
