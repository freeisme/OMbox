...
- 发布前运行 Python 编译、`cd frontend && pnpm build`（内含 `vue-tsc` 类型检查）、
  `tests/integration/qa_security_regression.py` 和浏览器关键路径验证。
- 测试与文档里的固定数据一律使用**虚拟信息**（张三 / 李四 / 测试部门 / IT-PC-01 / 测试仓库 等），
  不得引用真实人员姓名、工号、设备编码、组织或仓库名称；对外发布前按
  [发布前脱敏检查](release-safety.md) 再核对一遍。

## 文档导航

GitHub Wiki 提供适合团队日常查阅的模块化文档：

- [项目 Wiki](https://github.com/freeisme/OMbox/wiki)
...
- [运维与安全](https://github.com/freeisme/OMbox/wiki/Operations-and-Security)
- [发布与版本](https://github.com/freeisme/OMbox/wiki/Releases-and-Versioning)
...
├── server.py
├── frontend/
│   ├── src/views/              # Vue 页面（19 个导航页 + 登录页）
│   ├── src/components/ui/      # 共享骨架组件
│   ├── src/api/                # 接口封装
│   └── src/styles/             # 设计令牌与公共类
├── web/
│   ├── app/                    # pnpm build 产物（不入库）
...
前端是 Vue 3 + TypeScript + Vite + Element Plus 的单页应用，源码在 `frontend/src/`：

| 目录 | 作用 |
| --- | --- |
| `src/api/` | 接口封装（`client.ts` 负责 CSRF、401 与会话过期回调），视图不直接拼 URL |
| `src/views/` | 19 个导航页面 + 登录页 |
| `src/components/ui/` | 共享骨架组件：`PageHeader`、`FilterBar`、`ToolbarActions`、`DataPanel`、`StatusTag`、`EmptyHint`、`MetricCard`、`FormDialog`、`DetailDrawer` |
| `src/composables/` | `useConfirm`（危险操作确认）、`useBreakpoint`（四档断点） |
| `src/layouts/AppShell.vue` | 左侧导航 + 顶栏 + 内容区 |
| `src/styles/tokens.css` | 设计令牌：间距、字号、圆角、表格行高、面层色 |
| `src/styles/app.css` | 公共类与四档断点 |
| `src/navigation.ts` | 菜单、路径、标题与权限规则的唯一来源 |
| `src/router/index.ts` | 路由表与守卫，页面组件全部懒加载 |
| `src/labels.ts` | 状态与设备类型标签的唯一映射 |

构建产物写入 `web/app/`，由 `server.py` 提供静态文件与 SPA 回落。
Element Plus 按需引入，首屏 JS 约 268 KB（gzip 约 114 KB）。

浏览器本地只保存界面偏好；业务数据的正式来源是 MySQL：

- `oa-theme`：`light` 或 `dark`
- `oa-theme-color`：主题色
- `oa-workbench-*`：工作台模块顺序与显隐

会话过期（接口返回 401）会清空前端会话并跳回 `/login?redirect=<原地址>`。
列表筛选统一在筛选区操作，不需要「输入草稿 + 显式查询」的旧约定。
...
$env:MYSQL_BIN = "<MySQL 客户端完整路径>"
$env:MYSQLDUMP_BIN = "<mysqldump 完整路径>"
...
- 员工编号必须唯一。新增人员时工号可以留空，服务端会按所属组织生成
  `<组织编码>-NN`（例如 `IT-01`，取该前缀下第一个未占用的序号）；公用人员用
  `SHARED-<组织编码>-NN`。编辑时清空工号不会重新编号（公用人员除外），
  工号留空又没有选组织时会直接报错，避免生成无归属的编号。
...
回收目标仓库只对显示屏和非资产物资要求：办公终端回收是**直接回到办公终端台账的闲置设备**
（`it_asset_status = idle`），不进任何仓库，所以离职清单里不给办公终端出现仓库下拉。
...
1. 在 `frontend/src/views/` 新建 `XxxView.vue`，数据走 `frontend/src/api/` 下的封装，
   不要直接在视图里拼 URL。
2. 在 `frontend/src/navigation.ts` 的 `NAV_ITEMS` 加一条（路径、标题、分组、权限模块）。
3. 在 `frontend/src/router/index.ts` 的 `VIEW_LOADERS` 登记
   `page: () => import("../views/XxxView.vue")`。
4. 页面模板按列表骨架写：`PageHeader → FilterBar → ToolbarActions → DataPanel → 分页`；
   间距、字号、颜色只取 `tokens.css` 的令牌，不写裸 px 与十六进制色值。
5. 弹窗用 `FormDialog`，详情用 `DetailDrawer`，危险操作用 `useConfirm`。
6. 如果页面需要数据库数据，优先复用既有 `/api/*`；列表数据量大时补服务端分页接口，
   数据范围过滤必须复用 `filter_state_payload`。
7. 检查管理员、操作员、只读用户三种权限下的按钮显示和后端拒绝行为。
8. 检查亮色、暗色与四档断点（1440 / 1100 / 900 / 420）下的布局。
...
1. 在视图里用 `reactive` 维护表单模型，字段类型与 `frontend/src/api/` 的 payload 接口一致。
2. 打开弹窗时先做权限判断（`hasPermission(resource, "update")`），无权限不显示入口。
3. 提交前做前端即时校验，减少无效请求。
4. 调用 `api/` 里的命令函数（PUT/POST），不要直接改本地状态。
5. 成功后 `ElMessage.success` 并重新拉取数据；失败时保留用户输入并显示服务端错误。
...
1. 优先使用 `tokens.css` 里的变量与 `--el-*` 变量。
2. 需要在浅色/深色下取不同值时，在 `:root` 与 `html.dark` 中同时定义。
3. 检查输入框、禁用按钮、表格 hover、弹窗和错误提示。
4. **覆盖 Element Plus 组件样式时要提高特异性**：组件样式按需加载，可能晚于
   `tokens.css`，所以写 `.el-table.el-table--small ...` 这类两级类名。
5. 用浏览器无痕窗口验证首次加载和刷新后的主题保持。
...
cd frontend && pnpm build     # vue-tsc 类型检查 + vite 构建
```
