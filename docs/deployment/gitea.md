...
DEPLOY_LOCAL_GITEA_HTTP_ORIGIN=http://<Gitea 服务器地址>:3001
DEPLOY_LOCAL_GITEA_SSH_ORIGIN=ssh://git@<Gitea 服务器地址>:2222
```
...
| 来源 | 行为 | 适用 |
| --- | --- | --- |
| 内置 GitHub 地址 | 由更新服务直接 `git fetch https://github.com/<owner>/<repo>.git` | 服务器能访问 github.com |
| 使用部署目录 `origin` | 从部署检出的远端（通常就是内网 Gitea）取版本 | 服务器不能访问 github.com |

**实测常见情况**：内网服务器可以访问 `api.github.com`、`codeload.github.com`，但
`github.com` 本身不通（`git ls-remote https://github.com/...` 报
`Empty reply from server`）。此时内置 GitHub 来源会失败，请改用 `origin`，或在
「设置 → 系统设置」里把 `update_repository_url` 指向内网 Gitea 仓库地址。

同一个限制也意味着：Gitea 无法把 GitHub 当作**拉取镜像**的源（Gitea 跑在同一台服务器上，
同样访问不到 github.com）。可行的做法是**推送同步**——在能同时访问 GitHub 和内网 Gitea 的
机器上，一次推送两个远端：

```bash
git remote add github https://github.com/<owner>/OMbox.git
git remote add gitea  ssh://git@GITEA_HOST:2222/<owner>/OMbox.git
git push github main --tags && git push gitea main --tags
```

如果以后服务器能出网访问 github.com，可以在 Gitea 里把该仓库改成镜像并填写 GitHub 地址。
