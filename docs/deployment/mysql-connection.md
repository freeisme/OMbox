...
容器 `1Panel-mysql-Xwiv`，不是 OMbox 数据库。
...
部署账号当前没有 Docker Socket 权限，因此 Docker 命令使用 `sudo`。
...
- 应用容器：`healthy`
- MySQL 容器：`healthy`
- Docker DNS：应用容器通过服务名 `db` 解析到 MySQL 容器，容器内网地址随环境变化，不应写死
- 应用健康接口：HTTP `200`
- 数据库探针：`databaseProbe = 1`
- 必需表数量：以当前 `server.py` 的健康检查为准（随版本变化）
