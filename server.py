...
def build_state_payload(include_audit_logs: bool = True) -> dict:
    # 工作台汇总不需要审计日志（它是整包里最大的一块），按需跳过。
    audit_logs_query = (
        f"""
        SELECT COALESCE(JSON_ARRAYAGG(JSON_OBJECT(
          'id', CAST(audit_log_id AS CHAR),
...
# ---------------------------------------------------------------- 工作台汇总
WORKBENCH_RECENT_LIMIT = 5
WORKBENCH_INBOUND_LIMIT = 8
WORKBENCH_NOTIFICATION_LIMIT = 5
WORKBENCH_QUALITY_LIMIT = 5
# 工作台是首屏概览：默认 15 秒内复用同一账号的结果，
# 前端点"刷新数据"时带 refresh=1 绕过缓存。
WORKBENCH_CACHE_TTL_SECONDS = max(
    0, int(os.environ.get("WORKBENCH_CACHE_TTL_SECONDS", "15"))
)
WORKBENCH_CACHE: dict[str, tuple[float, dict]] = {}
WORKBENCH_CACHE_LOCK = threading.Lock()


def workbench_can_view(context: dict, module_code: str) -> bool:
    for item in PERMISSIONS.permissions(context):
        if text_value(item.get("moduleCode")) == module_code and parse_bool(item.get("canView")):
            return True
    return False


def workbench_collect(loader: Callable[[], object]) -> list[dict]:
    """工作台容错：无权限或单模块失败按空列表处理，不影响其它模块。"""
    try:
        result = loader()
    except Exception:  # 权限不足、模块未启用等都按"无数据"处理
        return []
    return [item for item in (result or []) if isinstance(item, dict)]


def workbench_computer_row(
    computer: dict,
    ...
    return {
        ...
        "deviceName": text_value(computer.get("deviceName")),
        "orgId": org_id,
        "orgName": (org_names or {}).get(org_id, ""),
        "deviceType": text_value(computer.get("deviceType")),
        "brand": text_value(computer.get("brand")),
        "model": text_value(computer.get("model")),
        "location": text_value(computer.get("location")),
        "status": text_value(computer.get("status")),
        "userId": user_id,
        "userName": (employee_names or {}).get(user_id, ""),
        "registeredDate": text_value(computer.get("registeredDate")),
    }


def workbench_movement_row(log: dict) -> dict:
    return {
        "id": text_value(log.get("id")),
        "typeName": text_value(log.get("typeName")),
        "brandName": text_value(log.get("brandName")),
        ...
        "quantity": max(1, sql_int(log.get("quantity"), 1)),
        "sourceLabel": text_value(log.get("sourceLabel")),
        "occurredAt": text_value(log.get("occurredAt")),
    }


def build_workbench_summary(context: dict) -> dict:
    """工作台首屏汇总：只返回计数与少量最近记录。

    复用 /api/state 的权限与数据范围过滤，保证与列表页口径一致；
    但跳过审计日志等大块数据，响应体积从数百 KB 降到十几 KB。
    """
    state = filter_state_payload(build_state_payload(include_audit_logs=False), context)
    computers = list(state.get("computers") or [])
    employees = list(state.get("employees") or [])
    orgs = list(state.get("orgs") or [])
    movement_logs = list(state.get("inventoryMovementLogs") or [])
    # 名称在服务端解析，前端不必再拉完整组织/人员列表
    org_names = {text_value(item.get("id")): text_value(item.get("name")) for item in orgs}
    employee_names = {
        text_value(item.get("id")): text_value(item.get("name")) for item in employees
    }

    def count_status(value: str) -> int:
        return sum(1 for item in computers if text_value(item.get("status")) == value)

    employee_id = text_value((context.get("employee") or {}).get("employeeId"))
    my_assets = [
        item for item in computers if employee_id and text_value(item.get("userId")) == employee_id
    ]
    recent_computers = sorted(
        computers, key=lambda item: text_value(item.get("registeredDate")), reverse=True
    )[:WORKBENCH_RECENT_LIMIT]
    recent_inbound = sorted(
        [item for item in movement_logs if text_value(item.get("direction")) == "increase"],
        key=lambda item: text_value(item.get("occurredAt")),
        reverse=True,
    )[:WORKBENCH_INBOUND_LIMIT]

    summary: dict = {
        "generatedAt": datetime.now().isoformat(timespec="seconds"),
        "stateRevision": sql_int(state.get("stateRevision"), 0),
        "assets": {
            "computers": len(computers),
            "inUse": count_status("in_use"),
            "idle": count_status("idle"),
            "attention": sum(
                1
                for item in computers
                if text_value(item.get("status")) in {"repair", "lost", "retired"}
            ),
            "employees": sum(1 for item in employees if text_value(item.get("status")) == "active"),
            "nonAssetItems": sum(
                max(0, sql_int(item.get("quantity"), 0))
                for employee in employees
                for item in (employee.get("nonAssetItems") or [])
            ),
            "rootOrgs": sum(1 for item in orgs if not text_value(item.get("parentId"))),
        },
        "recentComputers": [
            workbench_computer_row(item, org_names, employee_names) for item in recent_computers
        ],
        "recentInbound": [workbench_movement_row(item) for item in recent_inbound],
        "myAssets": [workbench_computer_row(item, org_names, employee_names) for item in my_assets],
    }

    service: dict = {}
    if workbench_can_view(context, "tickets"):
        tickets = workbench_collect(lambda: DOMAIN_ROUTER.tickets.list_tickets(context, {}))
        status_counts: dict[str, int] = defaultdict(int)
        for item in tickets:
            status_counts[text_value(item.get("status")) or "unknown"] += 1
        service["ticketStatus"] = dict(status_counts)
        service["openTickets"] = sum(
            count
            for status, count in status_counts.items()
            if status not in {"closed", "resolved", "cancelled"}
        )
        service["slaBreached"] = sum(
            1 for item in tickets if text_value(item.get("slaState")) == "breached"
        )
    if workbench_can_view(context, "approvals"):
        approvals = workbench_collect(lambda: DOMAIN_ROUTER.service.list_approvals(context))
        service["pendingApprovals"] = sum(
            1 for item in approvals if text_value(item.get("status")) == "pending"
        )
    if workbench_can_view(context, "changes"):
        service["changes"] = len(
            workbench_collect(lambda: DOMAIN_ROUTER.service.list_changes(context))
        )
    if workbench_can_view(context, "problems"):
        service["problems"] = len(
            workbench_collect(lambda: DOMAIN_ROUTER.service.list_problems(context))
        )
    if service:
        summary["service"] = service

    if workbench_can_view(context, "inspection_management"):
        tasks = workbench_collect(lambda: DOMAIN_ROUTER.inspection.list_tasks(context, {}))
        summary["inspection"] = {
            "running": sum(1 for item in tasks if text_value(item.get("status")) == "running"),
            "submitted": sum(1 for item in tasks if text_value(item.get("status")) == "submitted"),
        }

    if workbench_can_view(context, "quality"):
        issues = workbench_collect(lambda: DOMAIN_ROUTER.quality.list_issues("open"))
        summary["quality"] = {
            "open": len(issues),
            "top": [
                {
                    ...
                    "severityLabel": text_value(item.get("severityLabel")),
                    "ruleLabel": text_value(item.get("ruleLabel")),
                    "entityTypeLabel": text_value(item.get("entityTypeLabel")),
                    "firstDetectedAt": text_value(item.get("firstDetectedAt")),
                }
                for item in issues[:WORKBENCH_QUALITY_LIMIT]
            ],
        }

    if workbench_can_view(context, "notifications"):
        notifications = workbench_collect(
            lambda: DOMAIN_ROUTER.service.list_notifications(context)
        )
        summary["notifications"] = {
            "unread": sum(1 for item in notifications if not parse_bool(item.get("isRead"))),
            "recent": [
                {
                    "id": text_value(item.get("id")),
                    ...
                    "isRead": parse_bool(item.get("isRead")),
                    "createdAt": text_value(item.get("createdAt")),
                }
                for item in notifications[:WORKBENCH_NOTIFICATION_LIMIT]
            ],
        }
...
def workbench_summary_cached(context: dict, refresh: bool = False) -> dict:
    """带短期缓存的工作台汇总：缓存按账号隔离，避免跨账号串数据。"""
    cache_key = text_value(context.get("id")) or text_value(context.get("username")) or "anonymous"
    if WORKBENCH_CACHE_TTL_SECONDS > 0 and not refresh:
        with WORKBENCH_CACHE_LOCK:
            cached = WORKBENCH_CACHE.get(cache_key)
        if cached and (time.monotonic() - cached[0]) < WORKBENCH_CACHE_TTL_SECONDS:
            return cached[1]
    payload = build_workbench_summary(context)
    if WORKBENCH_CACHE_TTL_SECONDS > 0:
        with WORKBENCH_CACHE_LOCK:
            WORKBENCH_CACHE[cache_key] = (time.monotonic(), payload)
            if len(WORKBENCH_CACHE) > 512:
                # 防御性清理，正常规模不会触发
                for key, _ in sorted(
                    WORKBENCH_CACHE.items(), key=lambda item: item[1][0]
                )[: len(WORKBENCH_CACHE) // 2]:
                    WORKBENCH_CACHE.pop(key, None)
    return payload
...
def filter_state_include(payload: dict, include: str) -> dict:
    """按 include=key1,key2 裁剪 /api/state 的返回字段。

    页面只需要其中几块数据时用它把整包（约 550 KB）砍到必要部分，
    stateRevision 永远保留，便于前端判断是否要刷新缓存。
    """
    keys = {part.strip() for part in text_value(include).split(",") if part.strip()}
    if not keys:
        return payload
    result = {key: value for key, value in payload.items() if key in keys}
    if "stateRevision" in payload:
        result["stateRevision"] = payload["stateRevision"]
    return result
...
def _subtree_org_ids(orgs: list[dict], root_id: str) -> set[str]:
    """组织子树（含自身）的 id 集合；与前端筛选“全部组织（含下级）”一致。"""
    children: dict[str, list[str]] = {}
    for org in orgs:
        children.setdefault(text_value(org.get("parentId")), []).append(text_value(org.get("id")))
    result: set[str] = set()
    stack = [text_value(root_id)]
    while stack:
        current = stack.pop()
...
def paginate_computers(payload: dict, params: dict[str, list[str]]) -> dict:
    """服务端分页：GET /api/list/computers。

    过滤条件与前端列表页一一对应（状态、组织子树、关键字、使用人），
    pageSize=0 表示“取全部”，供导出与全选使用。

    只有办公终端做服务端分页：使用人员页要按组织树统计人数，还要在离职转交与设备弹窗里
    用到全量数据；IT 物资是目录树。两者保留按需裁剪后的快照。

    两个给使用人员页用的补充参数：
    - userId=<人员id> / unassigned=1：设备弹窗按人取终端、取可分配终端；
    - fields=names：只回 id / userId / deviceName / brand / model，把「名下设备」一列的取数
      从 144 KB 压到约 24 KB（品牌与型号仍要保留，否则前端的关键字过滤会退化）。
    """

    def param(name: str) -> str:
        return text_value((params.get(name) or [""])[0])

    orgs = list(payload.get("orgs") or [])
    employees = list(payload.get("employees") or [])
    org_names = {text_value(org.get("id")): text_value(org.get("name")) for org in orgs}
    org_id = param("orgId").strip()
    scope = _subtree_org_ids(orgs, org_id) if org_id else None
    keyword = param("keyword").strip().lower()
    status = param("status").strip()
    user_id = param("userId").strip()
    unassigned = parse_bool(param("unassigned"))
    user_names = {
        text_value(employee.get("id")): (
            f"{text_value(employee.get('name'))} {text_value(employee.get('employeeNo'))}"
        )
        for employee in employees
    }
    rows: list[dict] = []
    for row in payload.get("computers") or []:
        if user_id and text_value(row.get("userId")) != user_id:
            continue
        if unassigned and text_value(row.get("userId")):
            continue
        if status and text_value(row.get("status")) != status:
            continue
        if scope is not None and text_value(row.get("orgId")) not in scope:
            continue
        if keyword:
            haystack = " ".join(
                [
                    text_value(row.get("deviceName")),
                    text_value(row.get("brand")),
                    text_value(row.get("model")),
                    text_value(row.get("fixedAssetCode")),
                    text_value(row.get("snSt")),
                    text_value(row.get("location")),
                    org_names.get(text_value(row.get("orgId")), ""),
                    user_names.get(text_value(row.get("userId")), ""),
                ]
            ).lower()
    ...
    if param("fields").strip() == "names":
        rows = [
            {
                "id": text_value(row.get("id")),
                "userId": text_value(row.get("userId")),
                "deviceName": text_value(row.get("deviceName")),
    ...
    total = len(rows)
    requested_size = sql_int(param("pageSize"), LIST_PAGE_SIZE_DEFAULT)
    if requested_size <= 0:
        return {"items": rows, "total": total, "page": 1, "pageSize": 0}
    page_size = max(1, min(LIST_PAGE_SIZE_MAX, requested_size))
    page = max(1, sql_int(param("page"), 1))
...
class AppHandler(SimpleHTTPRequestHandler):
    ...
    def handle_api(self) -> None:
            ...
            self.send_json(
                {
                    ...
                    "settings": {
                        "app_name": public_settings.get("app_name", "OMbox运维（O&M）盒子"),
        ...
        if parsed.path == "/api/state" and self.command == "GET":
            ...
            self.send_json(filter_state_include(payload, (params.get("include") or [""])[0]))
            return

        if parsed.path.startswith("/api/list/") and self.command == "GET":
            # 服务端分页列表：办公终端。权限与数据范围复用状态快照的裁剪逻辑，
            # 看不到的集合在 filter_state_payload 里已经被清空。
            context = require_auth(self)
            resource = parsed.path[len("/api/list/") :].strip("/")
            if resource != "computers":
                self.send_json({"error": "Not found"}, status=HTTPStatus.NOT_FOUND)
                return
            with DB_LOCK:
                payload = filter_state_payload(build_state_payload(include_audit_logs=False), context)
            self.send_json(paginate_computers(payload, params))
            return

        if parsed.path == "/api/workbench/summary" and self.command == "GET":
            context = require_auth(self)
            refresh = parse_bool((params.get("refresh") or [""])[0])
            with DB_LOCK:
                payload = workbench_summary_cached(context, refresh)
