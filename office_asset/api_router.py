...
from .datacenter_devices import DatacenterDeviceService
...
class DomainApiRouter:
    def __init__(self, deps: ApiDependencies) -> None:
        ...
        self.datacenter_devices = DatacenterDeviceService(
            deps.db,
            self.scope,
            deps.api_error,
            deps.conflict_error,
    ...
    def dispatch(self, handler: object, parsed: ParseResult, params: dict[str, list[str]]) -> bool:
        ...
        if path.startswith("/api/inventory/usage/") and path.endswith("/return") and method == "POST":
            ...
            return True
        if path.startswith("/api/inventory/usage/") and path.endswith("/scrap") and method == "POST":
            parts = path.split("/")
            if len(parts) != 7 or parts[3] != "usage" or parts[6] != "scrap":
                raise self.deps.api_error("Invalid inventory usage scrap path.")
            allocation_type, usage_record_id = parts[4], parts[5]
            context = self._write_context(handler, "scrap_management", "create")
            self.deps.require_permission(context, "inventory_operations", "view")
            send_json(
                self.assets.scrap_inventory_usage(
                    allocation_type,
                    usage_record_id,
                    self._payload(handler),
                    context,
        ...
        if path == "/api/scrap-reasons" and method == "GET":
            context = self._read_context(handler, "scrap_management")
            send_json({"reasons": self.assets.list_scrap_reasons(context)})
            return True
        if path == "/api/scrap-records" and method == "GET":
            context = self._read_context(handler, "scrap_management")
            kind = str((params.get("kind") or [""])[0])
            limit = str((params.get("limit") or ["500"])[0])
            send_json({"records": self.assets.list_scrap_records(context, kind, limit)})
            return True
        if path.startswith("/api/scrap-records/") and method == "GET":
            parts = path.split("/")
            if len(parts) != 4 or not parts[3]:
                raise self.deps.api_error("Invalid scrap record path.")
            context = self._read_context(handler, "scrap_management")
        ...
        if path == "/api/datacenter-devices" and method == "GET":
            context = self._read_context(handler, "rack_layout")
            send_json({"devices": self.datacenter_devices.list_devices(context, params)})
            return True
        if path == "/api/datacenter-devices" and method == "POST":
            context = self._write_context(handler, "rack_layout", "create")
            send_json(self.datacenter_devices.create_device(self._payload(handler), context), status=201)
            return True
        ...
        if path == "/api/device-types" and method == "GET":
            ...
            return True
        if path == "/api/datacenter-devices/import" and method == "POST":
            context = self._write_context(handler, "rack_layout", "create")
            send_json(self.datacenter_devices.import_workbook(self._payload(handler), context))
            return True
        if path == "/api/device-types" and method == "POST":
            context = self._write_context(handler, "rack_layout", "create")
            send_json(self.device_topology.create_device_type(self._payload(handler), context), status=201)
            return True
        if path.startswith("/api/device-types/"):
            parts = path.split("/")
            if len(parts) == 5 and parts[4] == "image" and method == "POST":
                catalog_id = parts[3]
                context = self._write_context(handler, "rack_layout", "update")
                send_json(
                    self.device_topology.save_device_type_image(
                        catalog_id, self._payload(handler), context
                    )
                )
                return True
            if len(parts) == 6 and parts[4] == "image" and parts[5] == "remove" and method == "POST":
                context = self._write_context(handler, "rack_layout", "update")
                send_json(
                    self.device_topology.remove_device_type_image(
                        parts[3], self._payload(handler), context
                    )
                )
                return True
            if len(parts) == 4 and method == "GET":
                context = self._read_context(handler, "rack_layout")
                send_json({"deviceType": self.device_topology.get_device_type(parts[3], context)})
                return True
        ...
        if path.startswith("/api/computers/") and path.endswith("/assignments/return") and method == "POST":
            ...
            return True
        if path.startswith("/api/computers/") and path.endswith("/scrap") and method == "POST":
            computer_id = path.split("/")[-2]
            context = self._write_context(handler, "scrap_management", "create")
            self.deps.require_permission(context, "it_assets", "view")
            send_json(
                self.assets.scrap_computer(
                    computer_id, self._payload(handler), context, self._idempotency_key(handler)
                ),
                status=201,
            )
