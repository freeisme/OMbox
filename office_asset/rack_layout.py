...
class RackLayoutService:
        ...
        placements = list(
            self.db.json(
                ...
                SELECT COALESCE(JSON_ARRAYAGG(JSON_OBJECT(
                  ...
                  'datacenterDeviceId', COALESCE(CAST(placement.datacenter_device_id AS CHAR), ''),
                ...
                FROM (
                  SELECT placement.placement_id, placement.rack_id, placement.source_kind,
                         placement.computer_id, placement.inventory_model_id,
                         placement.datacenter_device_id, placement.display_name,
    ...
    def available_devices(self, context: dict, params: dict[str, list[str]] | None = None) -> dict:
        """Un-racked devices come from the datacenter ledger only.

        Office computers and IT supplies are deliberately excluded: they are not
        rack-mounted equipment and must not appear in the rack palette.
        """
        params = params or {}
        ...
        devices = list(
            self.db.json(
                f"""
                SELECT COALESCE(JSON_ARRAYAGG(JSON_OBJECT(
                  'deviceId', CAST(device.device_id AS CHAR),
                ...
                FROM (
                  ...
                  LEFT JOIN rack_device_placement placement
                    ON placement.datacenter_device_id = device.device_id
                   AND placement.is_active = 1
                  WHERE {' AND '.join(filters)}
    ...
    def place_device(
        ...
        device_id = self.db.integer(payload.get("datacenterDeviceId"), 0)
        if device_id <= 0:
            raise self.api_error("请选择要上架的机房设备。")
        device = self.db.json(
            f"""
            SELECT JSON_OBJECT(
              ...
              'ownerLabel', device.owner_label,
              'status', device.status,
              'orgId', COALESCE(CAST(device.org_unit_id AS CHAR), '')
            )
            FROM datacenter_device device
        ...
        device_status = self.db.text(device.get("status"))
        if device_status == "installed":
            raise self.conflict_error("该机房设备已经在某个机柜上架，请先下架或直接移动。")
        if device_status != "stock":
            raise self.conflict_error(f"该机房设备当前状态为 {device_status}，只有“未上架”的设备可以上架。")
        self.scope.assert_org_access(context, self.db.integer(device.get("orgId"), 0))
        ...
        display_name = self.db.text(payload.get("displayName"))[:128] or self.db.text(device.get("name"))
        brand_model = self.db.text(payload.get("brandModel"))[:160] or self.db.text(device.get("brandModel"))[:160]
        owner_label = self.db.text(payload.get("ownerLabel"))[:128] or self.db.text(device.get("ownerLabel"))[:128]
        ...
        output = self.db.execute(
            ...
            INSERT INTO rack_device_placement (
              rack_id, source_kind, datacenter_device_id, display_name, brand_model,
            ...
            VALUES (
              ...
              {self.db.quote(face)},
              'in_use',
              {self.db.quote(self.db.text(device.get('assetCode'))[:128])},
              {self.db.quote(self.db.text(device.get('serialNumber'))[:128])},
            ...
            {self._audit_sql(
                ...
                None,
                {
                    ...
                    "uHeight": u_height,
                    "face": face,
                    "datacenterDeviceId": str(device_id),
                },
            )};
            UPDATE audit_log
            ...
            SET status = 'installed',
                rack_id = {rack_id_int},
                site_id = {self.db.integer(rack.get('siteId'), 0) if self.db.integer(rack.get('siteId'), 0) > 0 else 'NULL'}
    ...
    def remove_placement(self, placement_id: object, payload: dict, context: dict) -> dict:
        ...
        placement_id_int = self.db.integer(placement.get("id"), 0)
        datacenter_device_id = self.db.integer(placement.get("datacenterDeviceId"), 0)
        actor_id = self._actor_id(context)
        self.db.execute(
            f"""
            START TRANSACTION;
            ...
            SET status = 'stock',
                rack_id = NULL,
                site_id = NULL
            WHERE device_id = {datacenter_device_id if datacenter_device_id > 0 else 0}
