...
const summary = computed(() => `显示 ${logs.value.length} / 共 ${total.value} 条`);
...
<template>
  <PageHeader title="操作日志" description="按操作类别、人员、对象与时间区间查看资产与物资的变动记录">
    <template #actions>
      <el-button :loading="loading" @click="load">刷新日志</el-button>
      <el-button :disabled="!logs.length" @click="exportCsv">导出当前结果</el-button>
    </template>
    <template #filters>
      <FilterBar :summary="summary" :resettable="hasFilters" @reset="reset">
        <el-form label-position="top" class="oa-filter-form" @submit.prevent="load">
          <el-row :gutter="12">
            ...
            <el-col :xs="24" :sm="12" :md="3">
              <el-form-item label="操作类别">
                <el-select v-model="filters.category" placeholder="全部类别" clearable class="oa-full-width">
                  <el-option
            ...
            <el-col :xs="24" :sm="12" :md="3">
              <el-form-item label="对象范围">
                <el-select v-model="filters.entityType" placeholder="全部对象" clearable class="oa-full-width">
                  <el-option
  ...
  <DataPanel class="oa-panel-gap" :loading="loading">
    <el-table :data="pagedLogs" size="small" border stripe height="calc(100vh - 420px)" empty-text="暂无符合条件的操作日志">
      ...
      <el-table-column label="变更对象" min-width="200">
        <template #default="{ row }">
          ...
          <div class="oa-cell-sub">{{ auditEntityTypeLabel(row.entityType) }}</div>
        </template>
      </el-table-column>
      <el-table-column label="关联人员" width="150">
        <template #default="{ row }">
          <div v-if="row.employeeName">
            {{ row.employeeName }}
            <div class="oa-cell-sub">{{ row.employeeId }}</div>
          </div>
      ...
      <el-table-column label="操作人" width="120">
        <template #default="{ row }">
          ...
          <div class="oa-cell-sub">{{ row.source }}</div>
        </template>
      </el-table-column>
  ...
  <DetailDrawer v-model="detailVisible" title="日志详情" size="md">
...
.oa-filter-form :deep(.el-form-item) {
  margin-bottom: 0;
}
