...
const typeForm = ref<Record<string, unknown>>({});
const portText = ref("");
...
const hasPanelImage = computed(
  () => Boolean(deviceType.value?.imageFrontPath) && showImage.value && !imageFailed.value,
);
...
function inferPortKind(portType: string): RackPort["kind"] {
  const value = portType.toLowerCase();
  if (value.includes("sfp") || value.includes("base-x") || value.includes("fiber")) return "fiber";
  if (value.includes("iec") || value.includes("nema") || value.includes("power")) return "power";
  if (value.includes("console") || value.includes("rj-45 console")) return "console";
  return "network";
}

function openTypeDialog(): void {
  const current = deviceType.value;
  typeForm.value = current
    ? {
        catalogId: current.id,
        slug: current.slug,
        manufacturer: current.manufacturer,
        model: current.model,
        uHeight: Math.max(1, Math.round(Number(current.uHeight) || 1)),
        category: current.category,
      }
    : { catalogId: "", slug: "", manufacturer: "", model: "", uHeight: 1, category: "network" };
  portText.value = (current?.ports ?? [])
    .map((port) => `${port.name},${port.type}`)
    .join("\n");
  typeDialog.value = true;
}

async function loadTypeIntoForm(catalogId: string): Promise<void> {
  if (!catalogId) return;
  ...
  typeForm.value = {
    ...
    model: detail.model,
    uHeight: Math.max(1, Math.round(Number(detail.uHeight) || 1)),
    category: detail.category,
  };
  portText.value = (detail.ports ?? []).map((port) => `${port.name},${port.type}`).join("\n");
}

async function submitDeviceType(): Promise<void> {
  typeSaving.value = true;
  try {
    const ports = portText.value
      .split("\n")
      .map((line) => line.trim())
      .filter(Boolean)
      .map((line) => {
        const [name, type = ""] = line.split(/[,，\t]/).map((part) => part.trim());
        const kind = inferPortKind(type);
        return { name, type, kind, face: kind === "power" ? "rear" : "front" };
      })
      .filter((port) => port.name);
    const result = await saveDeviceType({ ...typeForm.value, ports });
    ElMessage.success(`型号已保存（${result.catalogId}）。`);
    await loadDeviceTypes();
    await selectNode(selectedNodeId.value);
    typeDialog.value = true;
  } catch (error) {
    ElMessage.error(`保存型号失败：${(error as Error).message}`);
  } finally {
    typeSaving.value = false;
  }
}

async function uploadImage(face: "front" | "rear", event: Event): Promise<void> {
  const input = event.target as HTMLInputElement;
  const file = input.files?.[0];
  const catalogId = String(typeForm.value.catalogId ?? "");
  if (!file || !catalogId) {
    ElMessage.warning("请先保存型号，再上传面板图。");
    return;
  }
  if (file.size > 2 * 1024 * 1024) {
    ElMessage.warning("图片请控制在 2MB 以内。");
    return;
  }
  try {
    ...
    const result = await uploadDeviceTypeImage(catalogId, {
      face,
      fileName: file.name,
      contentBase64: window.btoa(binary),
    });
    ElMessage.success(`面板图已上传：${result.path}`);
...
async function clearImage(face: "front" | "rear"): Promise<void> {
  const catalogId = String(typeForm.value.catalogId ?? "");
  if (!catalogId) return;
  try {
    await removeDeviceTypeImage(catalogId, face);
    ElMessage.success("面板图已删除。");
    await loadDeviceTypes();
    await selectNode(selectedNodeId.value);
  } catch (error) {
  ...
  <el-card shadow="never" v-loading="loading">
    <template #header>
      <div class="panel-toolbar">
        ...
        <el-switch v-if="deviceType?.imageFrontPath" v-model="showImage" active-text="显示面板图" />
        <div class="spacer" />
        <el-button v-if="canUpdate" size="small" @click="openTypeDialog">型号与面板图</el-button>
    ...
    >
      <el-form label-position="top" size="small">
        ...
        <el-form-item label="端口类型">
          <el-select v-model="portForm.kind" class="oa-full-width">
        ...
        <el-form-item label="状态">
          <el-select v-model="portForm.status" class="oa-full-width">
    ...
    <FormDialog
      ...
      size="sm"
      confirm-text="生成端口"
      @confirm="submitTemplateImport"
    >
      <el-form label-position="top" size="small">
        <el-form-item label="型号">
          <el-select v-model="templateCatalogId" filterable class="oa-full-width">
            <el-option
      ...
      <div class="empty-hint">
        型号库用于生成端口模板与面板排布，可在接口 <code>POST /api/device-types</code> 手工维护；
        没有型号时也可以逐条“新增端口”。
      </div>
    </FormDialog>

    <FormDialog v-model="typeDialog" title="型号与面板图" size="lg" :loading="typeSaving">
      <template #footer>
        <el-button @click="typeDialog = false">关闭</el-button>
        <el-button type="primary" :loading="typeSaving" @click="submitDeviceType">保存型号</el-button>
      </template>
      <el-form label-position="top" size="small">
        <el-form-item label="已有型号">
          <el-select
            v-model="typeForm.catalogId"
            filterable
            clearable
            class="oa-full-width"
            placeholder="选择已有型号进行编辑，或留空新建"
            @change="loadTypeIntoForm"
          >
            <el-option
              v-for="item in deviceTypes"
              :key="item.id"
        ...
        <el-form-item label="U 高 / 类别">
          ...
          <el-select v-model="typeForm.category" style="width: 220px; margin-left: 8px">
      ...
      <el-divider content-position="left">面板图片（PNG / JPEG / WebP，单张 2MB 以内）</el-divider>
      <div class="image-row">
        <div class="image-slot">
          <span>正面</span>
          <img v-if="deviceType?.imageFrontPath" :src="deviceType.imageFrontPath" alt="正面面板图" />
          <small v-else>未上传</small>
          <div>
            <input type="file" accept="image/png,image/jpeg,image/webp" @change="uploadImage('front', $event)" />
            <el-button v-if="deviceType?.imageFrontPath" link type="danger" size="small" @click="clearImage('front')">
              删除
            </el-button>
          </div>
        </div>
        <div class="image-slot">
          <span>背面</span>
          <img v-if="deviceType?.imageRearPath" :src="deviceType.imageRearPath" alt="背面面板图" />
          <small v-else>未上传</small>
          <div>
            <input type="file" accept="image/png,image/jpeg,image/webp" @change="uploadImage('rear', $event)" />
            <el-button v-if="deviceType?.imageRearPath" link type="danger" size="small" @click="clearImage('rear')">
              删除
            </el-button>
          </div>
        </div>
      ...
      <div class="empty-hint">
        上传前先保存型号；图片保存到 <code>web/assets/device-images/</code>，面板页会把端口热点叠在图片上。
      </div>
